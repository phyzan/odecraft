...
void set_mpreal_prec(mpfr_prec_t prec){
    ...
    // Sets the MPFR default *and* re-sizes the lazex backend's thread_local scratch buffers.
...
mpfr_prec_t get_default_prec(){
    // The lazex backend has no default of its own -- it re-sizes scratch buffers to match MPFR's,
