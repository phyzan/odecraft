#ifndef ODECRAFT_DELAUNAY_IMPL_HPP
#define ODECRAFT_DELAUNAY_IMPL_HPP
...
int DelaunayTri<NDIM>::find_simplex(const double* point) const {
    ...
    if (nd == 1) {
        ...
        for (int sidx = 0; sidx < num_simplices; ++sidx) {
            ...
            const double xmin = ndspan::min<double>(x0, x1);
            const double xmax = ndspan::max<double>(x0, x1);
...
#endif // ODECRAFT_DELAUNAY_IMPL_HPP