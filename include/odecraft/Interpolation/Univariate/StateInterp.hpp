...
public:
    ...
    void    expand_by_owning(pbox::Box<Interpolator<T, N>>&& interpolant) requires (_is_void);
...
private:
    ...
    // Storage is mutually exclusive, matching the if constexpr (_is_void) dispatch used by
    // every accessor: when INTERPOLATOR is the abstract base the interpolants are owned
    // polymorphically in boxes_, otherwise they are held by value in interps_. The unused
    // member collapses to an empty placeholder so that std::vector<Interpolator<T, N>> -- a
    // vector of an abstract type, which cannot be copied -- is never instantiated.
    struct _NoStorage{};

    std::conditional_t<_is_void, _NoStorage, std::vector<INTERPOLATOR>>                 interps_ = {};
    std::conditional_t<_is_void, std::vector<pbox::owner<Interpolator<T, N>>>, _NoStorage> boxes_ = {};
    int _dir = 0;
...
#endif // ODECRAFT_STATE_INTERP_HPP